...
function initRoutings(folderRouting){
    let listPackageExcept = ['image', 'testing'];
    let apisRequire = [];

    fs.readdirSync(folderRouting).map(filename => {
        if(!listPackageExcept.includes(filename)){
            fs.readdirSync(`${folderRouting}/${filename}/apis`).map(fileRoute => {
                apisRequire[apisRequire.length] = `${filename}/apis/${fileRoute.slice(0, -3)}`;
            });
        }
    });

    return apisRequire.map(api => require(`../packages/${api}`));
}

const allApis = initRoutings(path.resolve(process.cwd() + '/www/packages'));

const initAuths = [
    require('./role/public'),
    require('./role/private'),
    ...allApis
];
